...
class RenameDialog(tk.Toplevel):
    ...
    def build_gui(self):
        ...
        mixer_frame = tk.Frame(nb, bg=COLOR_BG)
        nb.add(mixer_frame, text="Mixer")
        self._build_mixer_tab(mixer_frame)
    ...
    # -----------------------------------------------------------------
    # NEUER MIXER‑TAB (größere Schrift, breitere Kästchen, Dry als 9. Zeile, keine Verbindungslinien)
    # -----------------------------------------------------------------
    # -----------------------------------------------------------------
    # NEUER MIXER‑TAB (mit Rahmen um Block 1 und Block 2)
    # -----------------------------------------------------------------
    def _build_mixer_tab(self, parent):
        canvas = tk.Canvas(parent, bg=COLOR_BG, highlightthickness=0)
        h_scroll = tk.Scrollbar(parent, orient="horizontal", command=canvas.xview, bg=COLOR_BG)
        v_scroll = tk.Scrollbar(parent, orient="vertical", command=canvas.yview, bg=COLOR_BG)
        ...
        total_w = tg_name_w + volpan_w + eq_w + comp_w + fx1_w + fx2_w + 3*gap
        ...
        canvas.create_text(x0+tg_name_w/2, header_y, text="TG", fill=BOX_FG, font=FONT_B, anchor="n")
        canvas.create_text(x0+tg_name_w+volpan_w/2, header_y, text="Vol / Pan", fill=BOX_FG, font=FONT_B, anchor="n")
        canvas.create_text(x0+tg_name_w+volpan_w+eq_w/2, header_y, text="EQ", fill=BOX_FG, font=FONT_B, anchor="n")
        canvas.create_text(x0+tg_name_w+volpan_w+eq_w+comp_w/2 +gap, header_y, text="Comp", fill=BOX_FG, font=FONT_B, anchor="n")
        canvas.create_text(x0+tg_name_w+volpan_w+eq_w+comp_w+fx1_w/2 +2*gap, header_y, text="FX1", fill=BOX_FG, font=FONT_B, anchor="n")
        canvas.create_text(x0+tg_name_w+volpan_w+eq_w+comp_w+fx1_w+fx2_w/2 +3*gap, header_y, text="FX2", fill=BOX_FG, font=FONT_B, anchor="n")
        ...
        for tg in range(1, 9):
            ...
            canvas.create_text(x0+tg_name_w/2, y, text=f"TG{tg} {name[:10]}", fill=fg, font=FONT_B, anchor="n")

            vol = data.get('volume', 0)
            pan = data.get('pan', 0)
            canvas.create_text(x0+tg_name_w+volpan_w/2, y, text=f"{vol} / {pan}", fill=fg, font=FONT, anchor="n")

            # EQ-Box
            canvas.create_rectangle(x0+tg_name_w+volpan_w, y-2, x0+tg_name_w+volpan_w+eq_w, y+16,
                                    fill=eq_bg, outline=LINE_COLOR)
            canvas.create_text(x0+tg_name_w+volpan_w+eq_w/2, y+6, text="EQ", fill=eq_fg, font=FONT, anchor="center")

            # Comp-Box
            canvas.create_rectangle(x0+tg_name_w+volpan_w+eq_w+gap, y-2, x0+tg_name_w+volpan_w+eq_w+comp_w+gap, y+16,
                                    fill=comp_bg, outline=LINE_COLOR)
            canvas.create_text(x0+tg_name_w+volpan_w+eq_w+gap+comp_w/2, y+6, text="Comp", fill=comp_fg, font=FONT, anchor="center")

            fx1 = data.get('fx1send', 0)
            canvas.create_text(x0+tg_name_w+volpan_w+eq_w+comp_w+fx1_w/2 +2*gap, y, text=f"{fx1}", fill=fg, font=FONT, anchor="n")

            fx2 = data.get('fx2send', 0)
            canvas.create_text(x0+tg_name_w+volpan_w+eq_w+comp_w+fx1_w+fx2_w/2 +3*gap, y, text=f"{fx2}", fill=fg, font=FONT, anchor="n")

        # Dry-Zeile
        dry_y = data_start_y + 8*row_h
        dry = self.bus_params.get('dry', '99')
        dry_center_x = x0 + tg_name_w + volpan_w + eq_w + comp_w + fx1_w + fx2_w/2 + 3*gap
        canvas.create_text(dry_center_x, dry_y, text=f"Dry: {dry}", fill=BOX_FG, font=FONT, anchor="n")
